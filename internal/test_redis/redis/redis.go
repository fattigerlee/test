...
func New(_addr string, _pass string, _prefix string) (*RedisObject, error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("str(%v) %v", str, err))
	}
...
func checkRedisValidMap(_keys string, _targetData *redis.StringStringMapCmd) map[string]string {
	...
	for key, val := range _targetData.Val() {
		...
		// 签名不匹配
		sign := Md5([]byte("Cache:__" + _keys + key))
